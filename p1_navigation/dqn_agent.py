...
from model import QNetwork, QNetworkConvolutional
...
class Agent():
    ...
    def __init__(self, state_size, action_size, seed, convolutional=False):
        ...
        # Q-Network
        # Edit - If convolutional, use QNetworkConvolutional class
        if convolutional:
    ...
    def step(self, state, action, reward, next_state, done):
        ...
        # Edit - Must rearrange memory to C x H x W for PyTorch
        if self.convolutional:
            # Rearrange memory to B x C x H x W
            state = np.transpose(state, (0, 3, 1, 2)).copy()
            next_state = np.transpose(next_state, (0, 3, 1, 2)).copy()
    ...
    def act(self, state, eps=0.):
        ...
        """
        # Edit - account for convolutional
        if not self.convolutional:
            state = torch.from_numpy(state).float().unsqueeze(0).to(device)
        else:
            # Must change from B x H x W x C to B x C x H x W
            # Must also make a copy to actually change the memory as
            # numpy.transpose returns a view
            state = np.transpose(state, (0, 3, 1, 2)).copy()
    ...
    def learn(self, experiences, gamma):
        ...
        loss = F.mse_loss(y_j, local_inference)
        loss.backward()
        self.optimizer.step()
